 ...
 */
public class Q1923_Longest_Common_Subpath 
{
	public int longestCommonSubpath(int n, int[][] paths) 
	{
		...
		Map<Integer, List<Integer>>[] graphs = new Map[row];
		...
		for (int i = 0; i < row; i++)
		{
			...
			globalMax = Math.min(globalMax, paths[i].length);
		}
		...
		for (int i = 1; i < row; i++)
		{
			...
			for (int preStart : preStarts)
			{
				...
				{
					if (paths[i].length - curStart < localMax)
					{
						break;
					}
				...
				// eliminate duplicated work, for example [1010101], [1010101]
				graphs[i].remove(paths[i-1][preStart]);
			}
