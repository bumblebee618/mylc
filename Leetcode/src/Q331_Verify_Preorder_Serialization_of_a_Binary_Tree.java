...
/******
 * 
One way to serialize a binary tree is to use pre-order traversal. When we encounter a non-null node, we record the node's minValue. If it is a null node, we record using a sentinel minValue such as #.
...
Each comma separated minValue in the string must be either an integer or a character '#' representing null pointer.
